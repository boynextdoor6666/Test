...
async function loadReviews() {
  ...
  try {
    const response = await reviewsAPI.getWorkerReviews(props.workerId)
    ...
    error.value = 'Не удалось загрузить отзывы о работнике'
  } finally {
    loading.value = false
  }
}
...
// Обновляем отзывы при изменении workerId
...
<template>
  <div class="reviews-section">
    ...
    <div v-else-if="reviews.length === 0" class="no-reviews">
      <p>Пока нет отзывов об этом работнике.</p>
    </div>
